...
# # First Books
#
# This notebook prepares a data set of book information for a prediction task to
# try to predict if a new author will publish a second book.  Michael Ekstrand
# uses it for teaching data science.
...
# %%
author_works = (
    book_authors.join(book_info, on="book_id")
...
# %% [markdown]
#
# Now we only want authors first works that were published since GoodReads
# started in 2007, and no later than 2012 to give the author time to have a new
# book before the data runs out in 2017:

# %%
first_work_stats = work_stats.join(author_first_work, on="work_id")
first_work_stats = first_work_stats.filter(
    (pl.col("pub_year") >= 2008) & (pl.col("pub_year") <= 2012)
)

# %% [markdown]
#
# Ok - now we have a table of first-work statistics.  We're going to take those
# authors, and find out how many total works they have in the data set.

# %%
author_nworks = (
    first_work_stats.select(["author_id"])
    .join(author_works, on="author_id")
    .groupby("author_id")
    .agg([pl.col("work_id").n_unique().alias("au_nbooks")])
)
