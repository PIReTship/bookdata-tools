...
fn check_name_decode(name: &str, exp_variants: &[&str]) {
    ...
    for n in exp_variants {
        assert!(
            dec_variants.contains(&(*n).to_owned()),
...
fn test_last_first_variants() {
    check_name_decode(
        ...
        &["Sequeira Moreno, Francisco", "Francisco Sequeira Moreno"],
    );
}
...
fn test_last_first_punctuation() {
    check_name_decode(
        "Jomaa-Raad, Wafa,",
        &["Wafa Jomaa-Raad", "Jomaa-Raad, Wafa"],
    );
}
...
fn test_first_last_year() {
    check_name_decode(
        ...
        &["Ditlev Reventlow, 1712-1783", "Ditlev Reventlow"],
    );
}
...
fn test_trailing_comma() {
    check_name_decode(
        "Miller, Pat Zietlow,",
        &["Pat Zietlow Miller", "Miller, Pat Zietlow"],
    );
}
...
fn test_locked() {
    check_name_decode(
        "!!!GESPERRT!!!Moro, Simone",
        &["Simone Moro", "Moro, Simone"],
    );
}

#[test]
fn test_single_initial() {
    check_name_decode("Navarro, P.", &["Navarro, P", "P Navarro"])
}

#[test]
fn test_leading_comma() {
    check_name_decode(", Engelbert", &["Engelbert"]);
}
