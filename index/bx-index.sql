...
--- #table bx.add_action
--- #step Index ratings
CREATE INDEX IF NOT EXISTS bx_rating_user_idx ON bx.raw_rating (user_id);
CREATE INDEX IF NOT EXISTS bx_rating_isbn_idx ON bx.raw_rating (isbn);
ANALYZE bx.raw_rating;

--- #step Extract ISBNs
INSERT INTO isbn_id (isbn)
  SELECT DISTINCT isbn
  FROM bx.raw_rating WHERE isbn NOT IN (SELECT isbn FROM isbn_id);
ANALYZE isbn_id;
