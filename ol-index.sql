...
CREATE INDEX author_key_idx ON ol.author (author_key);
ALTER TABLE ol.work ADD PRIMARY KEY (work_id);
CREATE INDEX work_key_idx ON ol.work (work_key);
ALTER TABLE ol.edition ADD PRIMARY KEY (edition_id);
CREATE INDEX edition_key_idx ON ol.edition (edition_key);
...
CREATE MATERIALIZED VIEW ol.work_meta
  AS SELECT work_id, work_key, length(work_data::text) AS work_desc_length
    FROM ol.work;
CREATE INDEX work_meta_work_idx ON ol.work_meta (work_id);
CREATE INDEX work_meta_key_idx ON ol.work_meta (work_key);

CREATE MATERIALIZED VIEW ol.edition_meta
AS SELECT edition_id, edition_key, length(edition_data::text) AS edition_desc_length
   FROM ol.edition;
CREATE INDEX edition_meta_edition_idx ON ol.edition_meta (edition_id);
CREATE INDEX edition_meta_key_idx ON ol.edition_meta (edition_key);
...
AS SELECT work_id, jsonb_array_elements_text(work_data->'subjects') AS subject
  FROM ol.work;
CREATE INDEX work_subject_work_idx ON ol.work_subject (work_id);
CREATE INDEX work_subject_subj_idx ON ol.work_subject (subject);
