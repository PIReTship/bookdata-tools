...
`cluster-ratings`
:   Combine BookCrossing ratings with [book clusters](cluster.qmd) to produce (user, cluster, rating) from the explicit-feedback ratings. BookCrossing implicit feedback entries (rating of 0) are excluded. Produces `bx/bx-cluster-ratings.parquet`{.file}.
