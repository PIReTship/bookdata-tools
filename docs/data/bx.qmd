...
`cluster-ratings`
:   Combine BookCrossing ratings with [book clusters](clusters.qmd) to produce (user, cluster, rating) from the explicit-feedback ratings. BookCrossing implicit feedback entries (rating of 0) are excluded. Produces `bx/bx-cluster-ratings.parquet`{.file}.

`cluster-actions`
:   Combine BookCrossing interactions with [book clusters](cluster.qmd) to produce (user, cluster) implicit-feedback records. These records include the BookCrossing implicit feedback entries (rating of 0). Produces `bx/bx-cluster-actions.parquet`{.file}.
...
:   The explicit-feedback ratings ($r_{ui} > 0$ from `bx/cleaned-ratings.csv`{.file}), with book clusters as the `item`s.

`bx/bx-cluster-actions.parquet`{#file-bx-cluster-ratings-parquet}
:   All user-item interactions from `bx/cleaned-ratings.csv`{.file}, with book clusters as the `item`s.
