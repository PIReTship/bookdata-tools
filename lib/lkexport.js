...
const queries = {
  amazon: 'SELECT user_id, book_id, rating FROM az_ratings JOIN az_users USING (user_key) JOIN isbn_book_id ON (asin = isbn)',
  'bx-explicit': 'SELECT user_id, book_id, rating FROM bx_ratings JOIN isbn_book_id USING (isbn) WHERE rating > 0',
