...
const queries = {
  amazon: 'SELECT user_id, book_id, median(rating) FROM az_ratings JOIN az_users USING (user_key) JOIN ol_isbn_book_id ON (asin = isbn) GROUP BY user_id, book_id',
  'bx-explicit': 'SELECT user_id, book_id, median(rating) AS rating FROM bx_ratings JOIN ol_isbn_book_id USING (isbn) WHERE rating > 0 GROUP BY user_id, book_id',
